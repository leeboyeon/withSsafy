...
import com.ssafy.withssafy.databinding.ItemTeambuildBinding
import com.ssafy.withssafy.src.dto.study.Study

class TeamBuildAdapter(var context: Context) : RecyclerView.Adapter<TeamBuildAdapter.TeamViewHolder>() , Filterable {
    var list = mutableListOf<Study>()
    var filteredList = list
    inner class TeamViewHolder(private val binding:ItemTeambuildBinding) : RecyclerView.ViewHolder(binding.root){
        fun bind(study: Study){
            binding.study = study
            if(study.user!!.id == ApplicationClass.sharedPreferencesUtil.getUser().id){
                binding.fragmentTeamBuildMore.visibility = View.VISIBLE
            }else{
                binding.fragmentTeamBuildMore.visibility = View.GONE
            }
            if(study.sbLimit == study.studyMembers?.size){
                binding.fragmentTeamBuildIngOred.setChipBackgroundColorResource(R.color.grey)
            }
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): TeamViewHolder {
        return TeamViewHolder(DataBindingUtil.inflate(LayoutInflater.from(parent.context), R.layout.item_teambuild,parent,false))
    }
    ...
    override fun onBindViewHolder(holder: TeamViewHolder, position: Int) {
        holder.apply {
            ...
            var more = itemView.findViewById<ImageView>(R.id.fragment_teamBuild_more)
    ...
    fun setItemClickListener(itemClickListener: TeamBuildAdapter.ItemClickListener){
        this.itemClickListener = itemClickListener
    }
    fun setDeleteClickListener(deleteClickListener: TeamBuildAdapter.DeleteClickListener) {
        this.deleteClickListener = deleteClickListener
    }
    fun setModifyClickListener(modifyClickListener: TeamBuildAdapter.ModifyClickListener){
